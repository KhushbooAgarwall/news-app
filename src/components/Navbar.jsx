        ...
        </div>
        {
            open && (
                <div className='md:hidden px-4 pb-4'>
                    {
                        links.map((link) =>{
                            return <Link key={link} to={`/${link.toLowerCase()}`} onClick={() => setOpen(false)} className='block py-2 text-gray-700  dark:text-gray-200 dark:hover:text-white hover:text=blue-600 transition'>
                                {link}
                            </Link>
                        })
                    }
